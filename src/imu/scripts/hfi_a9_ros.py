...
# searching ttyUSB* device
def find_ttyUSB():
    print('imu serial is /dev/ttyUSB0, if multiple serial port devices are identified, please change the imu port in launch file')
    posts = [port.device for port in serial.tools.list_ports.comports() if 'USB' in port.device]
    print('the num of connection of device {} is {} : {}'.format('USB', len(posts), posts))
...
def handleSerialData(raw_data):
    ...
    if data_right_count > 200000:
        print("Error in sending data, exit")
        exit(0)
    ...
    if key < buff[2] + 5:
        ...
        if buff[2] == 0x2c and pub_flag[0]:
            ...
            print("this handling function doesn't provide " + str(buff[2]) + " analyse")
            print("or data error")
            buff = {}
            key = 0
        ...
        print(angle_degree[0])
        print(angle_degree[1])
        print(angle_degree[2])
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hf_imu.isOpen():
            rospy.loginfo("\033[32m serial port opening succeed...\033[0m")
        else:
            hf_imu.open()
            rospy.loginfo("\033[32m serial port opening succeed...\033[0m")
    except Exception as e:
        print(e)
        rospy.loginfo("\033[31m serial port opening failed\033[0m")
        exit(0)
    else:
        ...
        while not rospy.is_shutdown():
            try:
                ...
                print("imu lost connection，or wire broken")
                exit(0)
            else:
                if buff_count > 0:
